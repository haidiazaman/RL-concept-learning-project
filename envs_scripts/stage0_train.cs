...
public class stage0_train : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        Vector3[] positions = {new Vector3(-4,1*objectsScaleFactor/2,5),new Vector3(4,1*objectsScaleFactor/2,5),new Vector3(-6,1*objectsScaleFactor/2,-5),new Vector3(6,1*objectsScaleFactor/2,-5)};  // initialise the 4 fixed positions

        // int random_pos = Random.Range(0,4); // get random int
        
        List<int> assigned_pos = new List<int> {0, 1, 2, 3};
        assigned_pos = assigned_pos.OrderBy( x => Random.value ).ToList();

        Target = Instantiate(targetAttributes.obj, positions[assigned_pos[0]], Quaternion.identity); // instantiate the Target GO by using the selected prefab and randomly spawn a position using previous random int
        ...
        target_material.color=targetAttributes.color;

...
public class stage2 : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // get the color and prefab for target first
        int target_prefab_index = Random.Range(0,5);
        GameObject target_prefab = prefabList[target_prefab_index];
        Renderer renderer1 = Target.GetComponent<Renderer>();
        renderer1.material = material1;
        target_color_index = Random.Range(0,5);
        material1.color=colorList[target_color_index];
        Debug.Log("target prefab: "+target_prefab.name+" target color: "+target_color_index); // print out the name of the target_prefab, might need to save to a json file
        ...
        // for stage 2, check that type and color not same as target, but type or color can be same as target
        // assign the prefab type and material directly at 1 time
        // need to ensure the rest of the objects dont have the same colour as Target
        int otherObject1_prefab_index = Random.Range(0,5);
        int otherObject1_color_index = Random.Range(0,5);
        while (otherObject1_prefab_index==target_prefab_index && otherObject1_color_index==target_color_index)
        {
            otherObject1_prefab_index = Random.Range(0,5);
            otherObject1_color_index = Random.Range(0,5);
        }
        GameObject other_object_prefab1 = prefabList[otherObject1_prefab_index];
        material2.color=colorList[otherObject1_color_index];

        int otherObject2_prefab_index = Random.Range(0,5);
        int otherObject2_color_index = Random.Range(0,5);
        while (otherObject2_prefab_index==target_prefab_index && otherObject2_color_index==target_color_index)
        {
            otherObject2_prefab_index = Random.Range(0,5);
            otherObject2_color_index = Random.Range(0,5);
        }
        GameObject other_object_prefab2 = prefabList[otherObject2_prefab_index];
        material3.color=colorList[otherObject2_color_index];

        int otherObject3_prefab_index = Random.Range(0,5);
        int otherObject3_color_index = Random.Range(0,5);
        while (otherObject3_prefab_index==target_prefab_index && otherObject3_color_index==target_color_index)
        {
            otherObject3_prefab_index = Random.Range(0,5);
            otherObject3_color_index = Random.Range(0,5);
        }
        GameObject other_object_prefab3 = prefabList[otherObject3_prefab_index];
        material4.color=colorList[otherObject3_color_index];
    ...
    void OnCollisionEnter(Collision collided_object)
    {
        ...
        Material collided_object_material = renderer.material;
        Color collided_object_color = collided_object_material.color;
        if (collided_object.gameObject.name == Target.name && collided_object_color==material1.color) // this refers to the target object specifically since only the target is of the target object color
        {
            SetReward(10f); // SetReward is generally used when you finalise the task (before EndEpisode)
             // you can instead use AddReward() but this is used more for intermediate task, for example, need hit the target 5 times in a row before considerered complete, then maybe for loop then AddReward(0.2f), then SetReward(1.0f) then finally EndEpisode()
            GameObject.Destroy(Target);
        ...
        if (collided_object.gameObject.name == otherObject1.name && collided_object_color==material2.color) 
        {
            SetReward(-1.0f);
        }

        if (collided_object.gameObject.name == otherObject2.name && collided_object_color==material3.color) 
        {
            SetReward(-1.0f);
        }

        if (collided_object.gameObject.name == otherObject3.name && collided_object_color==material4.color) 
        {
            SetReward(-1.0f);
        }
    }
